...
import (
	...
	"github.com/benleb/gloomberg/internal/degendata/scorer"
...
func analyzeLawlessTokenNames(client *ethclient.Client) {
	...
	tokens := make(map[string]degendata.Metadata, 0)
	...
	for componentName, component := range components {
		for _, kind := range degendata.SortMapByValue(component, true)[:int(math.Min(float64(maxTokenLines), float64(len(component))))] {
			var fmtKind string
...
func getLawlessMetadata(client *ethclient.Client) []degendata.Metadata {
	...
	if !viper.GetBool("fresh") {
		if allMetadata, err := degendata.ReadMetadataFromFile(lawlessMetadataFile); err == nil {
	...
	scores := make(map[int64]degendata.Score, 0)
	...
	if content, err := ioutil.ReadFile(strings.Replace(lawlessMetadataFile, ".zstd.gob", ".json", 1) + "-scores"); err == nil {
		...
		rawScores := make([]degendata.Score, 0)
	...
	for i := big.NewInt(0); i.Cmp(totalSupply) < 0; i.Add(i, big.NewInt(1)) {
		...
		// unmarshal json
		var metadataJSON degendata.Metadata
	...
	// gob.Register(degendata.MetadataAttribute{})

	lg.Print("\n")
	lg.Print(len(allMetadata))
	lg.Print("\n")

	degendata.WriteDataToFile(allMetadata, lawlessMetadataFile)

	return allMetadata
}

func TestOwnScorer() {
	metadata, err := degendata.ReadMetadataFromFile(lawlessMetadataFile)
	if err == nil {
		log.Print("found metadata file ✓")
	} else if os.IsNotExist(err) {
		log.Info("metadata file does not exist, fetching from blockchain")
