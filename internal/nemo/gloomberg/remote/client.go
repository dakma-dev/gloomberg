...
func NewClient() *ClientGRPC {
	...
	client := connect(grpcAddress, opts)
	if client == nil {
		return nil
	}

	clientGRPC = &ClientGRPC{grpcAddress, opts, client}
...
func FetchEvents(gb *gloomberg.Gloomberg) {
	...
	for {
		if failedAttempts > 0 {
			...
			log.Printf("retrying to connect to gRPC in %.0f seconds...", waitTime.Seconds())
		...
		// grpcClient := connect(grpcAddress, opts)

		if grpcClient == nil {
			log.Errorf("fail to connect to gRPC %s", style.BoldAlmostWhite(grpcClient.grpcAddress))
		...
		stream, err := grpcClient.GetEvents(context.Background(), subsriptionRequest, grpc.WaitForReady(true))
		if err != nil {
...
func connect(grpcAddress string, opts []grpc.DialOption) gen.GloombergClient {
	// connect
