...
func GetAllAssetCollections(wallets *wallet.Wallets, nodes *nodes.Nodes) map[common.Address]*collections.GbCollection {
	allAssets := make(map[common.Address]*collections.GbCollection)
	...
	for _, w := range *wallets {
		...
		go func(w *wallet.Wallet) {
			...
			for _, asset := range GetAssetsFor(w.Address, nodes) {
				if existingAsset := allAssets[asset.ContractAddress]; existingAsset != nil {
					existingAsset.OwnedTokenIDs = append(existingAsset.OwnedTokenIDs, asset.OwnedTokenIDs...)
				} else {
					allAssets[asset.ContractAddress] = asset
				}
			}
		}(w)
	}
...
func GetAssetsFor(walletAddress common.Address, nodes *nodes.Nodes) map[common.Address]*collections.GbCollection {
	receivedAssets := make([]models.Asset, 0)
	receivedCollections := make(map[common.Address]*collections.GbCollection, 0)
	...
	for ok := true; ok; ok = cursor != "" {
		...
		if assetsResponse := fetchAssets(walletAddress, cursor); assetsResponse != nil {
			...
			receivedAssets = append(receivedAssets, assetsResponse.Assets...)
		} else {
			gbl.Log.Debug("error while fetching assets, skipping...")
		}
	}

	gbl.Log.Debugf("fetched all assets for %s | receivedAssets: %d\n", walletAddress.Hex(), len(receivedAssets))

	for _, asset := range receivedAssets {
		contractAddress := common.HexToAddress(asset.AssetContract.Address)

		if contractAddress == external.ENSContract || asset.Collection.Name == "MegaCryptoPolis" {
			gbl.Log.Debugf("contractAddress == external.ENSContract = %v | asset.Collection.Name == MegaCryptoPolis = %v\n", contractAddress == external.ENSContract, asset.Collection.Name == "MegaCryptoPolis")
			continue
		}
		...
		collection.OpenseaSlug = asset.Collection.Slug

		if tokenID, err := strconv.ParseUint(asset.TokenID, 10, 64); err == nil {
			(receivedCollections[contractAddress]).OwnedTokenIDs = append(receivedCollections[contractAddress].OwnedTokenIDs, tokenID)
		}
	}
...
func fetchAssets(walletAddress common.Address, cursor string) *models.AssetsResponse {
	...
	// create a variable of the same type as our model
	var assetsResponse *models.AssetsResponse
	...
	// decode the data
	if err := json.NewDecoder(bytes.NewReader(responseBody)).Decode(&assetsResponse); err != nil {
		gbl.Log.Errorf("⌛️ error while decoding asset contract for %s: %s", walletAddress.Hex(), err)
		return nil
	}
