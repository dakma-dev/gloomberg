...
// 	PricePerItem        string
// 	FloorPrice          string
// 	PricePerItemColor   lipgloss.TerminalColor
// 	NumItemsColor       lipgloss.TerminalColor
// 	TrendIndicator      string
// 	TrendIndicatorColor lipgloss.TerminalColor
...
// 	switch m := data.(type) { //nolint:gocritic
// 	case EventMessage:
// 		return m
// 	}
...
// 	queueOutWeb   *chan *totra.TokenTransaction
// 	Nodes         *nodes.Nodes
// }
...
// func New(queueWeb *chan *totra.TokenTransaction, listenAddress string, nodes *nodes.Nodes, getGasPrice func() int) *EventStream {
...
// func startWorker(s *live.Socket, queueOutWeb *chan *totra.TokenTransaction) {
// 	gbl.Log.Infof("webWorker started for session %v", live.SessionID((*s).Session()))

// 	// 	default:
// 	for event := range *queueOutWeb {
// 		// for event := range *queueOutWeb {
// 		gbl.Log.Infof("webWorker session %+v - event: %+v", live.SessionID((*s).Session()), event)

// 		data := EventMessage{
// 			ID:                  live.NewID(),
// 			Time:                event.ReceivedAt.Format("15:04:05"),
// 			Typemoji:            event.Action.Icon(),
// 			EventType:           strings.ToLower(event.Action.String()),
// 			NumItems:            uint64(event.TotalTokens),
// 			Price:               fmt.Sprintf("%6.3f", event.GetPrice().Ether()),
// 			PricePerItem:        fmt.Sprintf("%6.3f", event.GetPrice().Ether()/float64(event.TotalTokens)),
// 		}

// 		if err := (*s).Broadcast(wnewmessage, data); err != nil {
// 			gbl.Log.Errorf("failed broadcasting new message: %s", err)
// 		}

// 		continue

// 		// // if !event.PrintEvent {
// 		// if event.StateInfo != nil && !event.StateInfo.PrintInStream {
// 		// 	gbl.Log.Debugf("outWeb discarded event: %+v", event)

// 		// 	continue
// 		// }

// 		priceEther, _ := utils.WeiToEther(event.PriceWei).Float64()
// 		priceEtherPerItem, _ := utils.WeiToEther(big.NewInt(int64(event.PriceWei.Uint64() / event.TxLogCount))).Float64()
...
// 		var NumItemsColor, pricePerItemColor lipgloss.TerminalColor

// 		switch {
// 		case event.TxLogCount > 7:
// 			NumItemsColor = style.AlmostWhiteStyle.GetForeground()
// 			pricePerItemColor = style.DarkWhiteStyle.GetForeground()
// 		case event.TxLogCount > 4:
// 			NumItemsColor = style.DarkWhiteStyle.GetForeground()
// 			pricePerItemColor = style.LightGrayStyle.GetForeground()
// 		case event.TxLogCount > 1:
// 			NumItemsColor = style.LightGrayStyle.GetForeground()
// 			pricePerItemColor = style.GrayStyle.GetForeground()
// 		default:
// 			NumItemsColor = style.GrayStyle.GetForeground()
// 			pricePerItemColor = style.DarkGrayStyle.GetForeground()
// 		}
...
// 		var trendIndicatorColor lipgloss.TerminalColor
...
// 		} else {
// 			trendIndicator = "⊗"
// 			trendIndicatorColor = style.DarkGrayStyle.GetForeground()
// 		}
